...
extern "C"
{
    ...
    void Bmatch_PrintPartition(Bmatch_Man_t *pMan, Abc_Ntk_t *pNtk1, Abc_Ntk_t *pNtk2);
...
void Bmatch_PrintMatching(Abc_Ntk_t *pNtk1, Abc_Ntk_t *pNtk2, vMatch &MI, vMatch &MO)
{
    ...
    for (int i = 0; i < MI.size(); ++i)
    {
        ...
        {
            if (i != MI.size() - 1)
                Abc_Print(1, " (%c%s, %s)", p.sign() ? '~' : '\0', Abc_ObjName(Abc_NtkPi(pNtk1, i)), Abc_ObjName(Abc_NtkPi(pNtk2, p.var())));
            else
                Abc_Print(1, " (%s, %s)", p.sign() ? "CONST0" : "CONST1", Abc_ObjName(Abc_NtkPi(pNtk2, p.var())));
        }
    }
    Abc_Print(1, "\n  MO:");
...
#define BUS_PRINT(TYPENAME, TYPE, BIO, pNtk)                                     \
    ...
    if (!pMan->BI1.empty() || !pMan->BO1.empty())
        Abc_Print(1, "Cir1:\n");
    if (!pMan->BI1.empty())
        BUS_PRINT(input, Pi, pMan->BI1, pNtk1);
    if (!pMan->BO1.empty())
        BUS_PRINT(output, Po, pMan->BO1, pNtk1);
    if (!pMan->BI2.empty() || !pMan->BO2.empty())
        Abc_Print(1, "Cir2:\n");
    if (!pMan->BI2.empty())
        BUS_PRINT(input, Pi, pMan->BI2, pNtk2);
    if (!pMan->BO2.empty())
        BUS_PRINT(output, Po, pMan->BO2, pNtk2);
...
#define PRINT_UNATE(unateMat, pNtk)                                                   \
    ...
    {                                                                                 \
        ...
        for (int k = 0; k < Abc_NtkPiNum(pNtk); ++k)                                  \
            printf(" %3s", Abc_ObjName(Abc_NtkPi(pNtk, k)));                          \
        printf("\n");                                                                 \
        for (int i = 0; i < Abc_NtkPoNum(pNtk); ++i)                                  \
        {                                                                             \
            ...
            {                                                                         \
                printf("   %c", unateMat[i][j] == 1 ? 'p' : unateMat[i][j] == 2 ? 'n' \
                                                        : unateMat[i][j] == 3   ? '.' \
                                                                                : ' '); \
            }                                                                         \
            printf("\n");                                                             \
...
void Bmatch_PrintPartition(Bmatch_Man_t *pMan, Abc_Ntk_t *pNtk1, Abc_Ntk_t *pNtk2)
{
#define PRUNE_PRINT(name, Partition, pNtk)                                \
    do                                                                    \
    {                                                                     \
        Abc_Print(1, "%s: ", name);                                       \
        ...
        {                                                                 \
            ...
            {                                                             \
                if (name[0] == 'i')                                       \
                    Abc_Print(1, "%s ", Abc_ObjName(Abc_NtkPi(pNtk, j))); \
                else                                                      \
                    Abc_Print(1, "%s ", Abc_ObjName(Abc_NtkPo(pNtk, j))); \
            }                                                             \
            Abc_Print(1, "}");                                            \
        }                                                                 \
        printf("\n");                                                     \
    ...
    PRUNE_PRINT("ipartition1", pMan->iPartition1, pNtk1);
    PRUNE_PRINT("ipartition2", pMan->iPartition2, pNtk2);
    PRUNE_PRINT("opartition1", pMan->oPartition1, pNtk1);
    PRUNE_PRINT("opartition2", pMan->oPartition2, pNtk2);
}
