...
void Bmatch_CalSuppInfo(vSuppInfo &vSuppInfo, vSupp &oFuncSupp, vSupp &oStrSupp)
{
    ...
    std::sort(vSuppInfo.begin(), vSuppInfo.end(), [](std::tuple<int, int, int> const &t1, std::tuple<int, int, int> const &t2)
              { return std::get<SUPPFUNC>(t1) < std::get<SUPPFUNC>(t2); });
}
...
void Bmatch_CalEqual(vEqual &oEqual, Abc_Ntk_t *pNtk)
{
    ...
    Abc_NtkForEachPo(pNtk, pNode1, i)
    {
        ...
        if (std::find(cal.begin(), cal.end(), index1) != cal.end())
            continue;
        ...
        if (add)
            oEqual.emplace_back(temp);
        if (add)
            cal.emplace_back(index1);
        temp.clear();
    }
}
